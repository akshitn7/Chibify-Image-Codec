...
st.title("📸 Huffman Image Compression")
...
with tab1:
    ...
    if img_file and st.button("Compress"):
        ...
        st.download_button(label="Download your compressed file", data=compressed, file_name= (img_file.name.split(".")[0] + ".huff"), mime="application/octet-stream")

#Decompress        
with tab2:
    compressed = st.file_uploader("Upload Image", type="huff", accept_multiple_files=False)
